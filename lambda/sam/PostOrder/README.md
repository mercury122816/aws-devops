...
```bash
PostOrder$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
PostOrder$ sam local start-api
PostOrder$ curl http://localhost:3000/
```
...
```bash
PostOrder$ sam logs -n HelloWorldFunction --stack-name PostOrder --tail
```
...
```bash
aws cloudformation delete-stack --stack-name PostOrder
```
